...
public class CartProducer {
    ...
    @Value("${spring.kafka.topics.cart}")
    ...
    @Async
    public void sendRollbackRequestToCart(CartRollbackRequest request) {
        if (request.getInternalCartResponseList() != null && !request.getInternalCartResponseList().isEmpty()) {
            CompletableFuture<SendResult<String, String>> future = kafkaTemplate.send(cartTopic, JsonMapperUtil.toString(request));

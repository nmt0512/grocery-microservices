...
public class ProductProducer {
    ...
    @Async
    public void sendRollbackRequestToProduct(ProductRollbackRequest request) {
        if (request.getDeductingProductList() != null && !request.getDeductingProductList().isEmpty()) {
            CompletableFuture<SendResult<String, String>> future = kafkaTemplate.send(productTopic, JsonMapperUtil.toString(request));
